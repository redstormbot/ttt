...
local function show_group_settingsmod(msg, data, target)
    ...
    if data[tostring(msg.to.id)]['settings']['lock_join'] then
    	lock_chat = data[tostring(msg.to.id)]['settings']['lock_join']
   	end
   	local lock_leave = "no"
    if data[tostring(msg.to.id)]['settings']['lock_leave'] then
    	lock_leave = data[tostring(msg.to.id)]['settings']['lock_leave']
   	end
   	local clean_service = "yes"
    if data[tostring(msg.to.id)]['settings']['clean_service'] then
    	clean_service = data[tostring(msg.to.id)]['settings']['clean_service']
   	end
...
-------------------------------
local function lock_group_clean(msg, data, target)
  if not is_momod(msg) then
    return "For Moderators only!"
  end
  local group_chat_lock = data[tostring(target)]['settings']['lock_chat']
...
----------------------------------------
local function lock_group_cleaner(msg, data, target)
  if not is_momod(msg) then
    return "For moderators only!"
  end
  local group_cleaner_lock = data[tostring(target)]['settings']['clean_service']
  if group_cleaner_lock == 'yes' then
    return 'Cleaner is already Enabled'
  else
    data[tostring(target)]['settings']['clean_service'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Cleaner has been Enabled'
  end
end

local function unlock_group_cleaner(msg, data, target)
  if not is_momod(msg) then
    return "For moderators only!"
  end
  local group_cleaner_lock = data[tostring(target)]['settings']['clean_service']
  if group_cleaner_lock == 'no' then
    return 'Cleaner is already Disabled'
  else
    data[tostring(target)]['settings']['clean_service'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Cleaner has been Disabled'
  end
end
...
local function unlock_group_leave(msg, data, target)
  ...
  if group_leave_lock == 'no' then
    return 'leave is already unlocked'
  else
...
local function lock_group_photomod(msg, data, target)
  ...
  if group_photo_lock == 'yes' then
    return 'Group photo are already locked'
  else
...
local function username_id(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if vusername == member then
      ...
      member_id = v.id
      if mod_cmd:lower() == 'promote' then
        return promote(receiver, member_username, member_id)
      elseif mod_cmd:lower() == 'demote' then
...
local function run(msg, matches)
  ...
  if matches[1]:lower() == 'chat_created' and msg.from.id == 0 then
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[1]:lower() == 'chat_add_user' then
      ...
      local rules = data[tostring(msg.to.id)][data_cat]
      local rules = 'Welcome to "' .. string.gsub(msg.to.print_name, '_', ' ')
